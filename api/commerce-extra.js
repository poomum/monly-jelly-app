...
// อัปโหลดไฟล์ตรงจากเบราว์เซอร์ไปที่ Vercel Blob เลย (ไม่ผ่านเซิร์ฟเวอร์นี้อีกต่อไป)
// วิธีนี้ข้ามข้อจำกัดขนาด request body ของ Vercel Function (4.5MB) ไปได้เลย เพราะ
// ไฟล์จริงไม่เคยถูกส่งมาที่ฟังก์ชันนี้เลย — ฟังก์ชันนี้ทำหน้าที่แค่ "ออกบัตรผ่าน"
// (token) ชั่วคราวให้เบราว์เซอร์เอาไปอัปโหลดตรงกับ Vercel Blob เอง
const MAX_UPLOAD_BYTES = 200 * 1024 * 1024; // 200MB (กว้างกว่าลิมิตเดิมมาก เพราะไม่ผ่านเซิร์ฟเวอร์แล้ว)
...
async function handleUploadFile(req, res) {
  ...
  // กว่า เป็นค่าเริ่มต้นของสโตร์ที่เชื่อมหลัง 1 มิ.ย. 2569) — handleUpload ต้องการ
  // BLOB_READ_WRITE_TOKEN เสมอสำหรับการออก token ให้ลูกค้าอัปโหลดตรง (เอกสาร
  // Vercel ระบุชัดว่าโหมด client upload ต้องมีตัวนี้ ต่อให้ใช้ OIDC สำหรับงานอื่นก็ตาม)
  if (!process.env.BLOB_READ_WRITE_TOKEN) {
    ...
    res.status(500).json({
      error: "ยังไม่ได้ตั้งค่า Vercel Blob Storage ให้โปรเจกต์นี้ (ขาด BLOB_READ_WRITE_TOKEN) — ไปที่ Vercel Dashboard → Storage → เช็คว่าเชื่อม Blob Store ไว้ถูกต้อง แล้ว Redeploy ใหม่",
      diagnostic: diag,
    });
    return;
  }
  ...
  try {
    const jsonResponse = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        // เฉพาะแอดมินที่ login แล้วเท่านั้นถึงจะขอ token อัปโหลดได้ (กันคนนอกมา
        // อัปโหลดขยะ/สแปมเข้า Blob Store โดยตรง — สำคัญมาก เพราะ token นี้เปิด
        // ทางให้เขียนไฟล์เข้า Blob ได้เลยโดยไม่ผ่านการตรวจสอบอื่นอีก)
        //
        // สำคัญ: ใช้ clientPayload แทน header x-admin-session (เดิมใช้ header
        // แต่ตรวจสอบแล้วพบว่า "ไม่ถึงเซิร์ฟเวอร์เลย" ในบางกรณี — อาจเป็นเพราะ
        // การส่งผ่าน rewrite ของ vercel.json ร่วมกับ SDK ของ Vercel Blob ไม่
        // รับประกันว่า custom header จะถูกส่งต่อไปถึงปลายทางเสมอ — clientPayload
        // เป็นกลไกที่ Vercel Blob ออกแบบมาให้ใช้ส่งข้อมูลกำหนดเองแบบนี้โดยเฉพาะ
        // เชื่อถือได้กว่ามาก)
        let receivedToken = null;
        try {
          const parsed = clientPayload ? JSON.parse(clientPayload) : {};
          receivedToken = parsed.adminToken || null;
        } catch (e) { /* clientPayload parse ไม่ได้ ถือว่าไม่มี token ส่งมา */ }

        const session = await validateSession(receivedToken);
        if (!session) {
          // แนบสถานะ token ที่เซิร์ฟเวอร์ได้รับจริง (แค่ 8 ตัวแรก กันเปิดเผย
          // token เต็มๆ) ไปด้วย เพื่อเช็คให้ชัดว่าได้รับ clientPayload มาจริงไหม
          // หรือได้รับมาแต่ session หมดอายุ/ไม่ตรงกับที่บันทึกไว้
          const tokenPreview = receivedToken ? `ได้รับ token (${String(receivedToken).slice(0, 8)}...) แต่ validate ไม่ผ่าน` : "ไม่ได้รับ clientPayload/adminToken มาเลย";
          throw new Error(`กรุณาเข้าสู่ระบบแอดมินก่อน [${tokenPreview}]`);
        }
        ...
        return {
          ...
          tokenPayload: JSON.stringify({ adminName: session.name }),
        };
      },
      onUploadCompleted: async ({ blob, tokenPayload }) => {
        // Vercel Blob เรียก callback นี้เองหลังอัปโหลดเสร็จจริง (ฝั่งเซิร์ฟเวอร์
        // อาจไม่เห็น callback นี้ตอนทดสอบใน local เพราะ Vercel เรียกกลับมาไม่ถึง
        // localhost ได้ — ทำงานได้ปกติเมื่อ deploy ขึ้น Vercel จริงแล้วเท่านั้น)
        try {
          const { adminName } = tokenPayload ? JSON.parse(tokenPayload) : {};
          await logAdminAction(adminName || "แอดมิน", "อัปโหลดไฟล์ popup", { url: blob.url });
        } catch (e) { /* เงียบไว้ ไม่ให้ล้มทั้ง callback เพราะ log พลาด */ }
      },
    });
    ...
    res.status(400).json({
      error: e.message || "อัปโหลดไฟล์ไม่สำเร็จ",
      diagnostic: {
        BLOB_READ_WRITE_TOKEN: !!process.env.BLOB_READ_WRITE_TOKEN,
        VERCEL_OIDC_TOKEN: !!process.env.VERCEL_OIDC_TOKEN,
        BLOB_STORE_ID: !!process.env.BLOB_STORE_ID,
