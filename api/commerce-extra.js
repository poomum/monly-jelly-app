  ...
  try {
    const jsonResponse = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname) => {
        ...
        const receivedToken = req.headers["x-admin-session"];
        const session = await validateSession(receivedToken);
        if (!session) {
          // แนบสถานะ token ที่เซิร์ฟเวอร์ได้รับจริง (แค่ 8 ตัวแรก กันเปิดเผย
          // token เต็มๆ) ไปด้วย เพื่อเช็คให้ชัดว่าได้รับ header มาจริงไหม
          // หรือได้รับมาแต่ session หมดอายุ/ไม่ตรงกับที่บันทึกไว้
          const tokenPreview = receivedToken ? `ได้รับ token (${String(receivedToken).slice(0, 8)}...) แต่ validate ไม่ผ่าน` : "ไม่ได้รับ header x-admin-session มาเลย";
