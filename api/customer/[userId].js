...
const { addLoyaltyPoint, buildLoyaltyRewardMessage, getAllPromotions, savePromotions, generateUniquePromoCode, LOYALTY_POINTS_TARGET, LOYALTY_REWARD_DISCOUNT, LOYALTY_REWARD_VALID_MONTHS, mergeCustomerAccounts, resolveCurrentUserId } = require("../../lib/promotions");
...
module.exports = async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    if (payload.action === "award_loyalty_point") {
      // สำคัญ: ตามรอยหา LINE ID ปัจจุบันก่อนเสมอ เผื่อ userId ที่ใช้เรียกมา
      // (เช่น จากปุ่มในหน้าออเดอร์เก่า) เป็น ID เก่าที่ถูกรวมบัญชีไปแล้ว
      const resolvedUserId = await resolveCurrentUserId(userId);
      const result = await addLoyaltyPoint(resolvedUserId);
      if (!result) {
      ...
      if (result.rewardCreated && process.env.LINE_CHANNEL_ACCESS_TOKEN) {
        ...
        linePush(resolvedUserId, [{ type: "text", text: msg }], process.env.LINE_CHANNEL_ACCESS_TOKEN).catch(() => {});
      }
      logAdminAction(session.name, "ให้แต้มสะสมย้อนหลัง", { userId: resolvedUserId, newPoints: result.points, rewardCreated: !!result.rewardCreated });
    ...
    if (payload.action === "merge_account") {
      ...
      const result = await mergeCustomerAccounts(oldUserId, newUserId, session.name);
      if (result.error) {
        res.status(400).json({ error: result.error });
        return;
      }

      logAdminAction(session.name, `รวมบัญชีลูกค้า ${oldUserId} -> ${newUserId}`, result);
    ...
    if (payload.action === "create_personal_discount") {
      // สำคัญ: ตามรอยหา LINE ID ปัจจุบันก่อนเสมอ เผื่อ userId ที่เรียกมา
      // (เช่น จากปุ่มในหน้าออเดอร์เก่า) เป็น ID เก่าที่ถูกรวมบัญชีไปแล้ว —
      // ถ้าสร้างคูปองผูกกับ ID เก่าไปตรงๆ ลูกค้าจะใช้คูปองนี้ไม่ได้เลย
      const resolvedUserId = await resolveCurrentUserId(userId);
      ...
      const customerRaw = await kv.get(`customer:${resolvedUserId}`);
      const customer = customerRaw ? (typeof customerRaw === "string" ? JSON.parse(customerRaw) : customerRaw) : { userId: resolvedUserId };
      ...
      const couponId = `cpn-personal-${resolvedUserId}-${Date.now()}`; // สร้างไว้ก่อน จะได้ผูกกับ promotion ได้ถูกต้อง
      ...
      promotions.push({
        ...
        active: true,
        restrictedToUserId: resolvedUserId,
      ...
      const couponsRaw = await kv.get(`coupons:${resolvedUserId}`);
      ...
      await kv.set(`coupons:${resolvedUserId}`, coupons);

      logAdminAction(session.name, "สร้างส่วนลดเฉพาะบุคคล", { userId: resolvedUserId, discount: discountAmount, code: promoCode, validDays });
      ...
      if (process.env.LINE_CHANNEL_ACCESS_TOKEN) {
        ...
        linePush(resolvedUserId, [{ type: "text", text }], process.env.LINE_CHANNEL_ACCESS_TOKEN).catch(() => {});
      }
