...
const { addLoyaltyPoint, buildLoyaltyRewardMessage, getAllPromotions, savePromotions, generateUniquePromoCode, LOYALTY_POINTS_TARGET, LOYALTY_REWARD_DISCOUNT, LOYALTY_REWARD_VALID_MONTHS } = require("../../lib/promotions");
...
module.exports = async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    // ── ย้ายข้อมูลลูกค้าจาก LINE ID เก่าไปยัง LINE ID ใหม่ ──
    // ใช้ตอนลูกค้าเปลี่ยนเครื่อง/ล้างแอป/login LINE ใหม่ ทำให้ได้ userId ใหม่
    // ที่ไม่ตรงกับของเดิม (บัญชี "แยก" กันไป) — ย้ายแต้มสะสม, คูปอง, และประวัติ
    // ออเดอร์ทั้งหมดจาก ID เก่ามารวมกับ ID ปัจจุบันที่ลูกค้าใช้งานอยู่จริง แทนที่
    // จะปล่อยให้แต้ม/คูปองเก่าติดค้างอยู่กับ ID ที่ใช้งานไม่ได้แล้วตลอดไป
    if (payload.action === "merge_account") {
      const oldUserId = String(payload.oldUserId || "").trim();
      const newUserId = userId; // ID ปลายทาง = ID ที่ระบุมาในนี้ (ID ปัจจุบันที่ใช้งานอยู่)

      if (!oldUserId) {
        res.status(400).json({ error: "กรุณาระบุ oldUserId ที่จะย้ายข้อมูลมา" });
        return;
      }
      if (oldUserId === newUserId) {
        res.status(400).json({ error: "LINE ID เก่ากับใหม่เป็นตัวเดียวกัน ไม่มีอะไรต้องย้าย" });
        return;
      }

      const oldRaw = await kv.get(`customer:${oldUserId}`);
      if (!oldRaw) {
        res.status(404).json({ error: "ไม่พบข้อมูลลูกค้าของ LINE ID เก่านี้เลยค่ะ" });
        return;
      }
      ...
      // ── 1) รวมแต้มสะสม (บวกกัน ไม่ใช่แทนที่) ──
      // ถ้ารวมแล้วครบเป้าพอดี ใช้ logic เดียวกับตอนได้แต้มปกติเป๊ะ (ออกรางวัล
      // + รีเซ็ตเหลือเศษ) เพื่อให้พฤติกรรมเหมือนกันไม่ว่าจะได้แต้มมาทางไหน
      ...
      if (combinedPoints >= LOYALTY_POINTS_TARGET) {
        ...
        promotions.push({
          ...
          createdAt: new Date().toISOString(), createdBy: "ระบบอัตโนมัติ (รวมบัญชี)",
        });
      ...
      // ── 2) ย้ายคูปองทั้งหมด (คูปองวันเกิด/รางวัลสะสมแต้ม/ส่วนลดเฉพาะบุคคล) ──
      // ต้องแก้ restrictedToUserId ในตัวโปรโมชั่นจริงด้วย ไม่ใช่แค่ในลิสต์คูปอง
      // เฉยๆ ไม่งั้นแม้ย้ายคูปองมาโชว์แล้ว จะยังใช้ไม่ได้อยู่ดี (เพราะระบบเช็ค
      // สิทธิ์จาก promotion.restrictedToUserId ตอนใช้จริง ไม่ใช่จาก coupon list)
      ...
      // ── 3) ย้ายประวัติออเดอร์ทั้งหมดมาอยู่ภายใต้ ID ปัจจุบัน ──
      // ให้ลูกค้าเห็นประวัติการสั่งซื้อครบต่อเนื่อง ไม่ขาดหายไปตอนสลับ ID
      ...
      // ── 4) อัปเดตโปรไฟล์ลูกค้าปลายทาง (เติมข้อมูลที่ขาด ไม่ทับของที่มีอยู่แล้ว) ──
      const mergedCustomer = {
        ...oldCustomer, // ใช้เป็นฐานสำหรับข้อมูลที่ ID ใหม่อาจยังไม่มี (เช่น birthday, couponTheme ที่เคยตั้งไว้)
        ...newCustomer, // ทับด้วยข้อมูลปัจจุบันของ ID ใหม่ (ให้ ID ใหม่เป็นค่าหลักถ้ามีขัดกัน)
        userId: newUserId,
      ...
      // ── 5) ทำเครื่องหมายว่า ID เก่าถูกรวมไปแล้ว (ไม่ลบทิ้ง เก็บไว้เป็นหลักฐาน) ──
      await kv.set(`customer:${oldUserId}`, { ...oldCustomer, mergedInto: newUserId, mergedAt: new Date().toISOString() });

      logAdminAction(session.name, `รวมบัญชีลูกค้า ${oldUserId} -> ${newUserId}`, {
        oldUserId, newUserId, ordersMoved, couponsMoved: oldCoupons.length, finalPoints, rewardCreated: !!mergeRewardCreated,
      });
      ...
      res.status(200).json({
        ...
        finalPoints,
        ordersMoved,
        couponsMoved: oldCoupons.length,
        rewardCreated: mergeRewardCreated,
      });
      return;
    }
    ...
    if (payload.action === "create_personal_discount") {
      ...
      const couponId = `cpn-personal-${userId}-${Date.now()}`; // สร้างไว้ก่อน จะได้ผูกกับ promotion ได้ถูกต้อง
      ...
      promotions.push({
        ...
        linkedCouponId: couponId, // สำคัญ: ไว้ sync สถานะ "ใช้แล้ว" กลับไปที่คูปองในหน้า "คูปองของฉัน"
        maxUses: 1,
  ...
  if (req.method === "PATCH") {
    ...
    const { phone, addr, name, email, birth, birthday, avatar, couponTheme, couponGreeting, couponFontSize, couponTextColor, couponFontFamily, couponDecorations } = payload;
    const VALID_FONT_SIZES = ["small", "medium", "large"];
    const VALID_FONT_FAMILIES = ["'Mitr',sans-serif", "'Kanit',sans-serif", "'Prompt',sans-serif", "'Pattaya',sans-serif", "'Chonburi',sans-serif"];
    ...
    const updated = {
      ...
      ...(VALID_FONT_FAMILIES.includes(couponFontFamily) ? { couponFontFamily } : {}),
      ...(Array.isArray(couponDecorations)
        ? { couponDecorations: couponDecorations.filter((d) => VALID_DECORATIONS.includes(d)).slice(0, 5) }
        : {}),
      updatedAt: new Date().toISOString(),
    };
