...
const { mergeCustomerAccounts } = require("../lib/promotions");
...
// ใช้ตอนยืนยันตัวตนก่อนเชื่อม LINE ใหม่ทับของเก่า — คนละคนอาจพิมพ์ที่อยู่
// เดียวกันคนละแบบ (ย่อ/ไม่ย่อ, เว้นวรรคไม่เหมือนกัน) จึงเช็คแค่ว่า "คำส่วน
// ใหญ่" ที่มีความหมาย (ยาวตั้งแต่ 3 ตัวอักษรขึ้นไป กันคำเชื่อมสั้นๆ ที่ไม่มี
// นัยสำคัญ เช่น "ที่", "ใน") ของที่อยู่หนึ่งไปเจอในอีกที่อยู่หนึ่งกี่เปอร์เซ็นต์
// ถ้าทับซ้อนกันเกินเกณฑ์ที่ตั้งไว้ ถือว่า "น่าจะเป็นคนเดียวกันจริง" พอจะเชื่อ
// ได้ในระดับหนึ่ง (ใช้ร่วมกับเบอร์โทรที่ต้องตรงเป๊ะอยู่แล้วเป็นหลัก)
function addressLooksLikeMatch(addr1, addr2) {
  ...
  return overlapRatio >= 0.4; // อย่างน้อย 40% ของคำในที่อยู่หนึ่งต้องเจอในอีกที่อยู่หนึ่ง
}
  ...
  for (const event of events) {
    try {
      ...
      if (event.type === "follow") {
        // สำคัญมาก: บันทึก userId ทันทีก่อนเลย ไม่ต้องรอดึง profile สำเร็จ
        // ก่อน — เดิมถ้า lineGetProfile() ล้มเหลวด้วยสาเหตุใดก็ตาม (LINE API
        // มีปัญหาชั่วคราว, rate limit, network glitch) ลูกค้าคนนั้นจะไม่ถูก
        // บันทึกเป็นสมาชิกในระบบเลยแม้แต่นิดเดียว ทั้งที่จริงๆ มี userId อยู่
        // แล้วตั้งแต่ event.source.userId ไม่จำเป็นต้องรอ profile ก่อนเลย —
        // แก้โดยบันทึก userId ก่อนเสมอ แล้วค่อยพยายามเติมชื่อ/รูปทีหลัง
        // (ถ้าดึงได้ก็ดี ถ้าไม่ได้ก็ไม่เป็นไร อย่างน้อย userId ก็ถูกจับไว้แล้ว)
        ...
        // ── พยายามดึงโปรไฟล์เพิ่ม (ชื่อ/รูป) แบบ best-effort ──
        // ถ้าล้มเหลว ไม่กระทบอะไรเลย เพราะ userId ถูกบันทึกไปแล้วข้างบน
        let profile = null;
        try {
          profile = await lineGetProfile(userId, accessToken);
          customer = {
            ...customer,
          ...
          console.error(`ดึงโปรไฟล์ไม่สำเร็จตอนแอดไลน์ (แต่ userId ${userId} ถูกบันทึกไว้แล้ว):`, profileErr);
        }

        const greetingName = profile ? `คุณ${profile.displayName}` : "คุณลูกค้า";
        await lineReply(
          event.replyToken,
          [
            {
              type: "text",
              text: `สวัสดีค่ะ ${greetingName}! 🍬\nยินดีต้อนรับสู่ Monly Jelly\nขนมเพื่อสุขภาพจากวัตถุดิบธรรมชาติแท้ 🌿`,
            },
            {
              type: "template",
      ...
      if (event.type === "message" && event.message.type === "text") {
        ...
        // ── "เชื่อมออเดอร์ #MJ-XXXXXX" → ผูกออเดอร์ที่สั่งแบบ guest เข้ากับ
        //    LINE ID ของคนที่ส่งข้อความนี้เข้ามา ──
        // สำคัญ: ต้องเช็คจุดนี้ "ก่อน" logic ติดตามออเดอร์ทั่วไปด้านล่าง เพราะ
        // ข้อความนี้ก็มีรูปแบบ MJ-XXXXXX ปนอยู่เหมือนกัน ถ้าไม่แยกก่อนจะหลุด
        // ไปเข้า path ติดตามสถานะเฉยๆ แทนที่จะเชื่อมบัญชีให้จริงๆ
        const linkOrderMatch = text.match(/เชื่อมออเดอร์.*?(MJ-[A-Z0-9]{4,8})/i);
        if (linkOrderMatch) {
          const targetOrderId = linkOrderMatch[1].toUpperCase();
          // ดึงเบอร์โทรที่แนบมาด้วย (ถ้ามี) ใช้เป็นตัวยืนยันตัวตนเวลาจะ
          // "เชื่อมใหม่" ทับ userId เดิมที่มีอยู่แล้ว (กันคนอื่นมาสวมสิทธิ์)
          const phoneMatch = text.match(/เบอร์\s*([0-9]{9,10})/);
          const providedPhone = phoneMatch ? phoneMatch[1] : null;
          // ดึงที่อยู่ที่แนบมาด้วย (ถ้ามี) — อยู่ท้ายสุดของข้อความเสมอ (ตามที่
          // ปุ่มในแอปสร้างให้) ใช้เทียบแบบผ่อนปรนร่วมกับเบอร์โทร ไม่ต้องตรงเป๊ะ
          const addrMatch = text.match(/ที่อยู่\s*(.+)$/);
          const providedAddr = addrMatch ? addrMatch[1].trim() : null;

          const orderRaw = await kv.get(`order:${targetOrderId}`);
          const order = orderRaw ? (typeof orderRaw === "string" ? JSON.parse(orderRaw) : orderRaw) : null;

          if (!order) {
            await lineReply(event.replyToken, [{ type: "text", text: `❌ ไม่พบออเดอร์ #${targetOrderId} ในระบบเลยค่ะ ลองเช็คเลขออเดอร์อีกครั้งนะคะ` }], accessToken);
          } else if (order.userId === userId) {
            await lineReply(event.replyToken, [{ type: "text", text: `✅ ออเดอร์ #${targetOrderId} เชื่อมกับ LINE นี้ไว้อยู่แล้วค่ะ ไม่ต้องทำอะไรเพิ่มเลย` }], accessToken);
          } else if (order.userId && order.userId !== userId) {
            // มี userId อื่นผูกอยู่แล้ว — อนุญาตให้ "เชื่อมใหม่" ทับได้ ถ้าเบอร์
            // โทรที่แนบมาตรงกับที่บันทึกไว้ในออเดอร์เท่านั้น (กรณี LINE เก่า
            // หลุด/เปลี่ยนบัญชีมาจริงๆ) ไม่ใช่กันไม่ให้ทำอะไรเลยเหมือนเดิม
            const orderPhone = (order.customer && order.customer.phone) ? String(order.customer.phone).replace(/[^0-9]/g, "") : "";
            const orderAddr = (order.customer && order.customer.addr) || "";
            const phoneOk = providedPhone && orderPhone && providedPhone === orderPhone;
            // ถ้าออเดอร์นี้ไม่มีที่อยู่บันทึกไว้เลย (เผื่อกรณีเก่าๆ) ไม่ควรบล็อก
            // แค่เพราะเช็คที่อยู่ไม่ได้ — ให้ผ่านเฉพาะเงื่อนไขเบอร์โทรพอ
            const addrOk = !orderAddr || addressLooksLikeMatch(providedAddr, orderAddr);
            if (phoneOk && addrOk) {
              // เบอร์ตรงกัน ยืนยันได้ว่าเป็นเจ้าของจริง — รวมบัญชีเต็มรูปแบบ
              // เลย (ย้ายแต้มสะสม คูปอง และออเดอร์อื่นๆ ทั้งหมดของบัญชีเก่ามา
              // ด้วย ไม่ใช่แค่แก้ userId ของออเดอร์นี้ใบเดียว)
              const mergeResult = await mergeCustomerAccounts(order.userId, userId, "ระบบอัตโนมัติ (ยืนยันตัวตนผ่านเบอร์โทรทาง LINE)");
              if (mergeResult.error) {
                await lineReply(event.replyToken, [{ type: "text", text: `❌ เชื่อมบัญชีไม่สำเร็จ: ${mergeResult.error}` }], accessToken);
              } else {
                await lineReply(
                  event.replyToken,
                  [{ type: "text", text: `✅ เชื่อม LINE ปัจจุบันสำเร็จแล้วค่ะ! 🎉\nย้ายแต้มสะสม คูปอง และประวัติออเดอร์ทั้งหมดมาให้เรียบร้อยแล้ว\nต่อจากนี้จะได้รับแจ้งเตือนสถานะและสะสมแต้มได้ตามปกติเลยค่ะ 💚` }],
                  accessToken
                );
              }
            } else {
              await lineReply(event.replyToken, [{ type: "text", text: `⚠️ ออเดอร์ #${targetOrderId} นี้เชื่อมกับบัญชี LINE อื่นไปแล้วค่ะ ถ้าเป็นเจ้าของออเดอร์จริงๆ (เช่น เปลี่ยนเครื่อง/เปลี่ยนบัญชี LINE มา) รบกวนกดปุ่ม "เชื่อม LINE ปัจจุบันใหม่" ในหน้าติดตามออเดอร์อีกครั้งนะคะ หรือติดต่อแอดมินโดยตรงค่ะ` }], accessToken);
            }
          } else {
            order.userId = userId;
            await kv.set(`order:${targetOrderId}`, order);
            await lineReply(
              event.replyToken,
              [{ type: "text", text: `✅ เชื่อมออเดอร์ #${targetOrderId} กับ LINE นี้สำเร็จแล้วค่ะ! 🎉\nต่อจากนี้จะได้รับแจ้งเตือนสถานะและสะสมแต้มได้ตามปกติเลยค่ะ 💚` }],
              accessToken
            );
          }
          continue;
