...
const { addLoyaltyPoint, buildLoyaltyRewardMessage } = require("../lib/promotions");
...
module.exports = async (req, res) => {
  ...
  if (order.status !== "paid" && order.userId) {
    ...
    try {
      loyaltyResult = await addLoyaltyPoint(order.userId);
      if (loyaltyResult && loyaltyResult.rewardCreated && loyaltyAccessToken) {
        const rewardMsg = buildLoyaltyRewardMessage(loyaltyResult.rewardCreated, process.env.LIFF_ID);
  ...
  // ── แจ้งแอดมินด้วยว่าออเดอร์นี้ชำระเงินสำเร็จแล้ว ──
  // สำคัญ: ก่อนหน้านี้แอดมินจะได้แจ้งเตือนแค่ตอน "ออเดอร์ใหม่เข้ามา" (ซึ่งจะ
  // เป็นสถานะ "รอชำระเงิน" เสมอ เพราะเพิ่งสั่งซื้อ ยังไม่ได้จ่าย) แต่ไม่เคยได้
  // รับแจ้งเตือนเลยตอนที่ลูกค้าจ่ายเงินจริงๆ สำเร็จ ทำให้ต้องเข้าไปเช็คในระบบ
  // แอดมินเองตลอด จุดนี้ปิดช่องว่างนั้น — แจ้งทันทีว่า "ชำระเงินแล้ว" ไม่ต้อง
  // กลับไปเช็คซ้ำอีก
  const adminLineIds = (process.env.ADMIN_LINE_USER_IDS || "").split(",").map((s) => s.trim()).filter(Boolean);
  if (adminLineIds.length > 0 && accessToken) {
    const customerName = (order.customer && order.customer.name) || "-";
    const adminPaidText =
      `✅ ชำระเงินแล้ว! ออเดอร์ #${id}\n\n` +
      `👤 ลูกค้า: ${customerName}\n` +
      `💰 ยอดที่ตรวจพบ: ฿${amount}\n` +
      `📅 รอบผลิต: ${order.round ? order.round.date : "-"}` +
      (accountMismatchNote ? `\n\n${accountMismatchNote}` : "");
    for (const adminId of adminLineIds) {
      linePush(adminId, [{ type: "text", text: adminPaidText }], accessToken).catch(() => {});
    }
  }
  ...
  res.status(200).json({
    ...
    loyaltyPoints: loyaltyResult ? loyaltyResult.points : undefined,
    rewardCreated: loyaltyResult ? loyaltyResult.rewardCreated : undefined,
  });
};
