...
let lastLinePushError = null; // เก็บ error ล่าสุดจาก LINE ไว้ให้ endpoint อื่นดึงไปโชว์ในหน้าแอดมินได้ตรงๆ ไม่ต้องเปิด Vercel Logs ทุกครั้ง
...
async function linePush(userId, messages, accessToken) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      lastLinePushError = `HTTP ${resp.status}: ${errText || "ไม่ทราบสาเหตุ"}`;
      console.error(`LINE push failed: HTTP ${resp.status} — ${errText}`);
      return false;
    }
    lastLinePushError = null;
    return true;
  } catch (e) {
    lastLinePushError = `เชื่อมต่อไม่สำเร็จ: ${e.message}`;
...
const { getAllPromotions, checkPromoValidity, calcDiscount, finalizePromoUsage, reclaimPromoUsage, addLoyaltyPoint, buildLoyaltyRewardMessage, mergeCustomerAccounts, resolveCurrentUserId, addressLooksLikeMatch } = require("../lib/promotions");
...
// ── ตรวจจับและรวมบัญชีเก่าอัตโนมัติ (LINE ID เปลี่ยนไป แต่เบอร์เดิม) ──
// สำคัญ: แก้ปัญหาที่ลูกค้าเปลี่ยนเครื่อง/ล้างแอป/login LINE ใหม่ ทำให้ได้ userId
// ใหม่ที่ไม่ตรงกับบัญชีเดิม — เดิมต้องให้แอดมินไปกดรวมบัญชีเองด้วยมือ ตอนนี้
// ระบบจะตรวจจับให้อัตโนมัติทุกครั้งที่มีออเดอร์ใหม่เข้ามา โดยหาว่ามี "บัญชี
// ลูกค้าเก่า" ที่เบอร์โทรตรงกัน แต่ userId คนละตัว (และยังไม่เคยถูกรวมไปที่
// ไหนมาก่อน) อยู่ไหม ถ้าเจอ จะรวมบัญชีให้ทันที ไม่ต้องรอแอดมินมากดเอง
async function autoDetectAndMergeOldAccount(newUserId, phone) {
  const normalizedPhone = String(phone).replace(/[^0-9]/g, "");
  if (!normalizedPhone) return;

  // ── สำคัญมาก: sync เบอร์โทรเข้าโปรไฟล์ของ ID ปัจจุบันก่อนเสมอ ──
  // เดิมระบบจะบันทึกเบอร์ลง customer:{userId} ก็ต่อเมื่อลูกค้าเข้าไปกรอกเอง
  // ในหน้าโปรไฟล์เท่านั้น ไม่เคย sync จากตอนสั่งซื้อเลย ทำให้การตรวจจับบัญชี
  // เก่า (เทียบเบอร์ข้าม customer record) หาไม่เจอเลยตั้งแต่ต้น เพราะไม่มีเบอร์
  // ให้เทียบทั้งฝั่งเก่าและฝั่งใหม่ — sync ตรงนี้ให้ครบก่อน ค่อยไล่หาบัญชีเก่าต่อ
  const selfRaw = await kv.get(`customer:${newUserId}`);
  const selfCustomer = selfRaw ? (typeof selfRaw === "string" ? JSON.parse(selfRaw) : selfRaw) : { userId: newUserId };
  if (selfCustomer.phone !== normalizedPhone) {
    selfCustomer.phone = normalizedPhone;
    selfCustomer.userId = newUserId;
    await kv.set(`customer:${newUserId}`, selfCustomer);
    await kv.sadd("customers:index", newUserId);
  }

  const customerIds = (await kv.smembers("customers:index")) || [];
  for (const candidateId of customerIds) {
    if (candidateId === newUserId) continue; // ตัวเองไม่นับ

    const raw = await kv.get(`customer:${candidateId}`);
    if (!raw) continue;
    const candidate = typeof raw === "string" ? JSON.parse(raw) : raw;
    if (candidate.mergedInto) continue; // รวมไปที่อื่นแล้ว ข้าม กันรวมซ้ำ/รวมวน

    const candidatePhone = String(candidate.phone || "").replace(/[^0-9]/g, "");
    if (!candidatePhone || candidatePhone !== normalizedPhone) continue;

    // เจอบัญชีเก่าที่เบอร์ตรงกัน แต่ userId คนละตัว → รวมบัญชีให้อัตโนมัติทันที
    console.log(`[auto-merge] ตรวจพบบัญชีเก่า ${candidateId} เบอร์ตรงกับ ${newUserId} (${normalizedPhone}) กำลังรวมบัญชีอัตโนมัติ`);
    const result = await mergeCustomerAccounts(candidateId, newUserId, "ระบบอัตโนมัติ (ตรวจพบเบอร์ตรงกัน)");
    if (result.error) {
      console.error(`[auto-merge] รวมบัญชีอัตโนมัติไม่สำเร็จ: ${result.error}`);
    } else {
      console.log(`[auto-merge] รวมบัญชีสำเร็จ: ${candidateId} -> ${newUserId} (แต้ม=${result.finalPoints}, คูปอง=${result.couponsMoved}, ออเดอร์=${result.ordersMoved})`);
    }
    return; // รวมได้แค่ 1 บัญชีเก่าต่อครั้ง (กรณีปกติไม่ควรมีมากกว่า 1 อยู่แล้ว)
  }
}
...
module.exports = async (req, res) => {
  ...
  if (req.method === "PATCH") {
    ...
    if (action === "resend_notification") {
      ...
      // สำคัญ: ตามรอยหา LINE ID ปัจจุบันก่อนเสมอ เผื่อออเดอร์นี้มี ID เก่าที่
      // ถูกรวมบัญชีไปแล้ว — ถ้าตามรอยแล้วเจอ ID ใหม่ที่ต่างจากเดิม ให้ใช้ ID
      // ใหม่ในการส่งเลย และบันทึกกลับเข้าออเดอร์ด้วย กันต้องตามรอยซ้ำทุกครั้ง
      const resolvedUserId = await resolveCurrentUserId(existing.userId);
      if (resolvedUserId !== existing.userId) {
      ...
      if (!sent) {
        // สำคัญ: เช็คโดยตรงว่า LINE ID นี้มีตัวตนจริงในระบบของ LINE ไหม (ทำใน
        // ฝั่งเซิร์ฟเวอร์เลย ในคำขอเดียวกันนี้ กันปัญหาที่เคยเจอ — เดิมให้หน้า
        // เว็บยิง fetch แยกต่างหากไปเช็คเอง แต่ error ถูกจับเงียบๆ ไปโดยไม่มี
        // อะไรโชว์เลยถ้าจุดนั้นพลาด ทำให้ไม่เห็นผลลัพธ์อะไรขึ้นมาเลย) ทำในนี้
        // เลยแทน รับประกันว่าจะมีผลตรวจสอบติดไปกับ error message เสมอ
        let profileCheckResult = "";
        try {
          const profileResp = await fetch(`https://api.line.me/v2/bot/profile/${encodeURIComponent(existing.userId)}`, {
            headers: { Authorization: `Bearer ${accessTokenResend}` },
          });
          const profileBodyText = await profileResp.text().catch(() => "");
          profileCheckResult = profileResp.ok
            ? `\n\n✅ เช็คแล้ว: LINE ID นี้มีตัวตนจริงในระบบ (HTTP ${profileResp.status}) — ปัญหาไม่ได้อยู่ที่ตัว ID เอง`
            : `\n\n❌ เช็คแล้ว: LINE ไม่รู้จัก ID นี้เลย (HTTP ${profileResp.status}: ${profileBodyText})\nสรุป: ID นี้ใช้งานไม่ได้จริงๆ ต้องใช้เครื่องมือ "รวมบัญชี" ย้ายไปหา ID ที่ใช้งานได้แทน`;
        } catch (e) {
          profileCheckResult = `\n\n⚠️ เช็ค ID เพิ่มเติมไม่สำเร็จ: ${e.message}`;
        }

        res.status(500).json({
          error: "ส่งข้อความไม่สำเร็จ" + (lastLinePushError ? `: ${lastLinePushError}` : " กรุณาลองใหม่อีกครั้ง") + profileCheckResult,
          // สำคัญ: ส่ง userId ที่ระบบเก็บไว้กลับไปด้วย ให้แอดมินเช็คเองได้เลยว่า
          // รูปแบบถูกต้องไหม (ควรขึ้นต้นด้วย U ตามด้วยตัวอักษร/ตัวเลข 32 หลัก)
          // โดยไม่ต้องเข้าไปดูในฐานข้อมูลเอง ช่วยวินิจฉัยปัญหาข้อมูลผิดพลาดได้เร็วขึ้น
          debugUserId: existing.userId,
        });
        return;
      }
    ...
    if (status === "paid" && existing.status !== "paid" && existing.userId) {
      try {
        const result = await addLoyaltyPoint(existing.userId);
    ...
    // ── ถ้ายกเลิกออเดอร์ที่เคยใช้โค้ดโปรโมชั่นไว้ → คืนสิทธิ์การใช้โค้ดให้ลูกค้า ──
    // สำคัญมาก: คูปองส่วนตัว (วันเกิด/สะสมแต้ม/VIP) ใช้ได้แค่ 1 ครั้งเท่านั้น
    // ถ้าไม่คืนสิทธิ์ตรงนี้ ลูกค้าจะเสียคูปองไปฟรีๆ ทั้งที่ไม่เคยได้ใช้ประโยชน์
    // จากส่วนลดจริงเลย (แค่เผลอยกเลิกออเดอร์ที่ยังไม่ได้จ่ายเงิน)
    if (status === "cancelled" && existing.status !== "cancelled" && existing.promoCode) {
      reclaimPromoUsage(existing.promoCode, id).catch((e) => console.error("reclaimPromoUsage failed:", e));
    }
    ...
    // ── แจ้งแอดมินยืนยันว่ายกเลิกออเดอร์สำเร็จแล้ว ──
    // สำคัญ: เดิมกดยกเลิกในหน้าแอดมินแล้วไม่มีอะไรยืนยันกลับมาทาง LINE เลยว่า
    // ทำสำเร็จจริงไหม ต้องเข้าไปเช็คในระบบแอดมินเองอีกรอบ — จุดนี้ปิดช่องว่างนั้น
    // เหมือนกับที่ทำไว้ตอน "ชำระเงินแล้ว" ก่อนหน้านี้
    if (status === "cancelled" && existing.status !== "cancelled") {
      const adminLineIds = (process.env.ADMIN_LINE_USER_IDS || "").split(",").map((s) => s.trim()).filter(Boolean);
      if (adminLineIds.length > 0 && accessToken) {
        const customerName = (existing.customer && existing.customer.name) || "-";
        const adminCancelText =
          `❌ ยกเลิกออเดอร์สำเร็จแล้ว! #${id}\n\n` +
          `👤 ลูกค้า: ${customerName}\n` +
          `💰 ยอดออเดอร์: ฿${existing.grandTotal}\n` +
          `📅 รอบผลิต: ${existing.round ? existing.round.date : "-"}` +
          (promoted ? `\n\n🎫 เลื่อนคิวสำรองขึ้นมาแทนแล้ว: #${promoted.orderId}` : "");
        for (const adminId of adminLineIds) {
          linePush(adminId, [{ type: "text", text: adminCancelText }], accessToken).catch(() => {});
        }
      }
    }
  ...
  // ── ลูกค้าเชื่อมออเดอร์เข้ากับ LINE ของตัวเองโดยตรง ──
  // ต่างจาก PATCH ด้านบนตรงที่ endpoint นี้ "ไม่ต้องใช้สิทธิ์แอดมิน" เลย
  // เพราะผู้เรียกคือตัวลูกค้าเอง (ผ่านปุ่มในแอป ยืนยันตัวตนด้วย LIFF อยู่แล้ว)
  // — ใช้เบอร์โทร + ที่อยู่ (แบบผ่อนปรน) เป็นชั้นความปลอดภัยแทนสิทธิ์แอดมิน
  // ทำงานแบบเดียวกับตอนส่งข้อความเข้าแชท LINE เป๊ะ แค่ไม่ต้องผ่านขั้นตอน
  // คัดลอก-วางข้อความเองแล้ว กดปุ่มเดียวจบในแอปได้เลย
  if (req.method === "POST" && req.query.action === "customer_link_order") {
    let body = {};
    try { const raw = await getRawBody(req); body = raw ? JSON.parse(raw) : {}; }
    catch { res.status(400).json({ error: "invalid json" }); return; }

    const { orderId: linkOrderId, userId: newUserId, phone: providedPhone, addr: providedAddr } = body;
    if (!linkOrderId || !newUserId) {
      res.status(400).json({ error: "ต้องระบุ orderId และ userId" });
      return;
    }
    const targetId = String(linkOrderId).trim().toUpperCase();
    const targetOrder = await kv.get(`order:${targetId}`);
    if (!targetOrder) {
      res.status(404).json({ error: "ไม่พบออเดอร์นี้ในระบบเลยค่ะ" });
      return;
    }

    if (targetOrder.userId === newUserId) {
      res.status(200).json({ ok: true, alreadyLinked: true, message: "เชื่อมกับ LINE นี้ไว้อยู่แล้วค่ะ" });
      return;
    }

    // ── เช็คว่าเป็นเจ้าของออเดอร์จริงไหม ก่อนอนุญาตให้เชื่อม/รวมบัญชี ──
    const orderPhone = (targetOrder.customer && targetOrder.customer.phone) ? String(targetOrder.customer.phone).replace(/[^0-9]/g, "") : "";
    const normalizedProvidedPhone = String(providedPhone || "").replace(/[^0-9]/g, "");
    const phoneOk = orderPhone && normalizedProvidedPhone && orderPhone === normalizedProvidedPhone;

    const orderAddr = (targetOrder.customer && targetOrder.customer.addr) || "";
    const addrOk = !orderAddr || addressLooksLikeMatch(providedAddr, orderAddr);

    if (!phoneOk) {
      res.status(403).json({
        error: "เบอร์โทรไม่ตรงกับที่บันทึกไว้ในออเดอร์นี้ค่ะ",
        debugOrderPhone: orderPhone || "(ไม่มีเบอร์บันทึกไว้ในออเดอร์เลย)",
        debugProvidedPhone: normalizedProvidedPhone || "(ไม่ได้ส่งเบอร์มาเลย)",
      });
      return;
    }
    if (!addrOk) {
      res.status(403).json({ error: "ที่อยู่ไม่ตรงกับที่บันทึกไว้ในออเดอร์นี้ค่ะ" });
      return;
    }

    if (!targetOrder.userId) {
      // ออเดอร์ guest แท้ๆ ไม่มีใครเป็นเจ้าของมาก่อน เชื่อมได้เลยไม่ต้อง merge
      targetOrder.userId = newUserId;
      await kv.set(`order:${targetId}`, targetOrder);

      // สำคัญมาก: ถ้าออเดอร์นี้จ่ายเงินไปแล้วตั้งแต่ตอนยังเป็น guest อยู่ (ตอน
      // จ่ายเงินไม่มี userId ให้ระบบรู้ว่าจะให้แต้มใคร) แต้มสะสมของรอบนั้นจะ
      // ไม่เคยถูกสร้างขึ้นมาเลย — พอเชื่อม LINE ID เข้ามาทีหลังแบบนี้ ต้องให้
      // แต้มย้อนหลังให้ตอนนี้เลย ไม่งั้นลูกค้าจะเสียแต้มไปตลอดกาล ทั้งที่จ่าย
      // เงินจริงและเชื่อมบัญชีถูกต้องแล้วก็ตาม
      let retroactiveReward = null;
      if (targetOrder.status === "paid" && !targetOrder.loyaltyPointAwarded) {
        try {
          const loyaltyResult = await addLoyaltyPoint(newUserId);
          if (loyaltyResult) {
            targetOrder.loyaltyPointAwarded = true;
            await kv.set(`order:${targetId}`, targetOrder);
            retroactiveReward = loyaltyResult.rewardCreated || null;
            const loyaltyAccessToken = process.env.LINE_CHANNEL_ACCESS_TOKEN;
            if (retroactiveReward && loyaltyAccessToken) {
              const rewardMsg = buildLoyaltyRewardMessage(retroactiveReward, process.env.LIFF_ID);
              linePush(newUserId, [{ type: "text", text: rewardMsg }], loyaltyAccessToken).catch(() => {});
            }
          }
        } catch (e) {
          console.error("ให้แต้มย้อนหลังตอนเชื่อมออเดอร์ guest ไม่สำเร็จ:", e);
        }
      }

      res.status(200).json({ ok: true, merged: false, retroactiveLoyaltyPointAwarded: !!retroactiveReward || (targetOrder.status === "paid"), rewardCreated: retroactiveReward });
      return;
    }

    // มี userId เก่าผูกอยู่แล้ว (LINE เก่าที่หลุด) → รวมบัญชีเต็มรูปแบบ
    const mergeResult = await mergeCustomerAccounts(targetOrder.userId, newUserId, "ระบบอัตโนมัติ (ลูกค้ายืนยันตัวตนในแอปโดยตรง)");
    if (mergeResult.error) {
      res.status(400).json({ error: mergeResult.error });
      return;
    }
    res.status(200).json({ ok: true, merged: true, ...mergeResult });
    return;
  }
  ...
  if (order.userId && order.customer && order.customer.phone) {
    ...
    // ตรวจจับ+รวมบัญชีเก่าอัตโนมัติ (LINE ID เปลี่ยนไปแต่เบอร์เดิม) — ทำงาน
    // แบบไม่บล็อกการตอบกลับเช่นกัน กันดีเลย์ออเดอร์ปัจจุบัน
    autoDetectAndMergeOldAccount(order.userId, order.customer.phone).catch((e) =>
      console.error("autoDetectAndMergeOldAccount failed:", e)
    );
  }
